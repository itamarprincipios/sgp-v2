...
- [x] **Fix falso negativo do DCRR (04/07/2026, commit `159fedb`):** a IANNE afirmava que EF05LP05/06 "não existiam no DCRR" porque o teto de 42k chars truncava as seções do polivalente (~6k cada). Agora: seções vão INTEIRAS (teto 150k; corte proporcional apenas se estourar, com aviso de truncamento no texto) e o prompt tem regra crítica: material é recorte → NUNCA afirmar inexistência, dizer "não localizada no trecho fornecido; confirmar no documento completo"; só invalidar habilidade pelo próprio código (EF+ano+componente). Polivalente 5º ano = 83k chars sem truncamento, verificado que contém EF05LP05.
- [x] **Parecer reformulado (04/07/2026, commit `20854c4`):** quinzena = 10 dias letivos seg-sex (nunca cobrar plano de fim de semana); estrutura nova do parecer: "⚠️ Erros graves" (só se houver: metodologia com erro grave, sem forma de avaliação, metodologia sem divisão em momentos, habilidade BNCC inexistente/de outro ano) → "Pontos a melhorar" (só ressalvas, sem elogios) → "Pontos positivos" → "Situação geral" em 1 frase; cruzamento objetos de conhecimento × habilidades contra o DCRR.
- [x] **Fix professores duplicados (04/07/2026, commit `d3a3640`):** confirmação em lote criava um professor por card; agora reaproveita cadastro pelo nome/escola. SQL de limpeza na seção 4b (executado em produção em 04/07/2026).
