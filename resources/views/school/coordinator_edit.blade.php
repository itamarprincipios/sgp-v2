    ...
    <div class="max-w-lg mx-auto bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8">
        ...
        <form action="{{ route('school.coordinator.update') }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="whatsapp" required value="{{ old('whatsapp', $coordinator->whatsapp) }}" placeholder="Ex: 5595999999999" class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                <p class="text-[10px] text-slate-500">Ao redefinir a senha, ela passa a ser os 4 últimos dígitos deste número.</p>
            </div>
