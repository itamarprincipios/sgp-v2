<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Editar Período de Planejamento') }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="text-lg font-bold text-slate-900">Editar Período de Envio</h3>
                <p class="text-xs text-slate-500">Ajuste o nome, vigência e prazos limite do planejamento pedagógico.</p>
            </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 md:p-8">
            <form action="{{ route('school.planning.update') }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="school_id" required class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                            @foreach($schools as $s)
                                <option value="{{ $s->id }}" {{ $planning->school_id == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                            @endforeach
                        </select>
                    </div>
                @else
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" id="name" required value="{{ $planning->name }}" placeholder="Ex: Planejamento Bimestral 01" class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="description" id="description" required value="{{ $planning->description }}" placeholder="Ex: Período de 02 a 13 de Março/2026" class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="start_date" id="start_date" required value="{{ \Carbon\Carbon::parse($planning->start_date)->format('Y-m-d') }}" class="w-full text-sm border border-slate-200 rounded-xl px-4 py-2.5 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                ...
                <!-- End Date -->
                <input type="hidden" name="end_date" value="{{ \Carbon\Carbon::parse($planning->end_date)->format('Y-m-d H:i:s') }}">
                ...
                <div class="border-t border-slate-100 pt-4 space-y-3">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_physical_education" id="is_physical_education" value="1" {{ $planning->is_physical_education ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_monitor" id="is_monitor" value="1" {{ $planning->is_monitor ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_first_grade" id="is_first_grade" value="1" {{ $planning->is_first_grade ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500">
                ...
                <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm shadow transition cursor-pointer">
