<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <form action="{{ route('semed.schools.update', $school) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $school->name) }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring focus:ring-indigo-500/20 text-sm py-2.5 transition @error('name') border-rose-500 focus:border-rose-500 focus:ring-rose-500/20 @enderror" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="inep_code" id="inep_code" value="{{ old('inep_code', $school->inep_code) }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring focus:ring-indigo-500/20 text-sm py-2.5 transition @error('inep_code') border-rose-500 focus:border-rose-500 focus:ring-rose-500/20 @enderror">
                        @error('inep_code')
                            <p class="text-xs text-rose-600 mt-1.5 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
