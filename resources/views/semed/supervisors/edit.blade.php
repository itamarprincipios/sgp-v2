<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-900 leading-tight">
            {{ __('Editar Supervisor(a): ' . $user->name) }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <form action="{{ route('semed.supervisors.update', $user) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 p-4 border border-slate-200 rounded-lg max-h-56 overflow-y-auto @error('school_ids') border-rose-500 @enderror">
                            @foreach($schools as $s)
                                <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                                    <input type="checkbox" name="school_ids[]" value="{{ $s->id }}" class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500" {{ in_array($s->id, old('school_ids', $assignedSchoolIds)) ? 'checked' : '' }}>
                                    {{ $s->name }}
                                </label>
                            @endforeach
                        </div>
